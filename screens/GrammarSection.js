// Updated GrammarSection Component with Japanese Language as Default
...
const languages = [
  { code: 'en', name: '英語', icon: '🇺🇸' },
  { code: 'kr', name: '韓国語', icon: '🇰🇷' },
  { code: 'ja', name: '日本語', icon: '🇯🇵' },
  { code: 'zh', name: '中国語', icon: '🇨🇳' },
  { code: 'vi', name: 'ベトナム語', icon: '🇻🇳' },
];
...
const GrammarCard = ({ item, selectedLanguage, theme, speakText }) => {
  ...
  return (
    <TouchableOpacity
      style={[styles.grammarCard, { backgroundColor: theme.cardBackground, borderColor: theme.primaryButtonColor }]}
      onPress={handlePress}
      activeOpacity={0.8}
    >
      {!showExample ? (
        <View style={styles.grammarView}>
          <Text style={[styles.grammarText, { color: theme.textColor }]}>{item.grammar}</Text>
          {selectedLanguage !== 'ja' && (
          ...
          <Text style={styles.tapInstruction}>
            <Ionicons name="hand-left" size={16} color={theme.textColor} /> タップして例文を見る
          </Text>
        </View>
      ) : (
        <View style={styles.exampleView}>
          <Text style={[styles.exampleText, { color: theme.textColor }]}>{item.example}</Text>
          {selectedLanguage !== 'ja' && (
            <View style={styles.translationsContainer}>
              <Text style={[styles.translationText, { color: theme.textColor }]}> {item.translations[selectedLanguage]}</Text>
            </View>
          )}
          <TouchableOpacity style={styles.speakButton} onPress={() => speakText(item.example)}>
  ...
  useEffect(() => {
    const initTTS = async () => {
      try {
        ...
        const japaneseVoice = voices.find(voice => voice.language === 'ja-JP');
        if (japaneseVoice) {
          await Tts.setDefaultVoice(japaneseVoice.id);
        }
      } catch (error) {
        console.warn('TTS 初期化エラー:', error);
      }
    };
  ...
  useLayoutEffect(() => {
    if (showGrammar) {
      navigation.setOptions({
        ...
        headerLeft: () => (
          <TouchableOpacity onPress={resetGrammarState} style={styles.closeButton}>
        ...
        headerRight: () => (
          <TouchableOpacity onPress={() => setShowLanguageModal(true)} style={styles.languageButton}>
            <Text style={[styles.languageButtonText, { color: theme.textColor }]}> {getLanguageName(selectedLanguage)}</Text>
          </TouchableOpacity>
        ),
      });
    } else {
      navigation.setOptions({ headerShown: false, tabBarStyle: { display: 'flex' } });
    }
    ...
    return (
      <View style={[styles.container, { backgroundColor: theme.backgroundColor }]}>
        <View style={styles.centerContainer}>
          <TouchableOpacity style={styles.mainButton} onPress={() => setShowLanguageModal(true)}>
            <Ionicons name="book" size={50} color={theme.textColor} />
            <Text style={[styles.text, { color: theme.textColor, marginTop: 10 }]}>文法学習を始める</Text>
          </TouchableOpacity>
        </View>
        ...
        >
          <SafeAreaView style={[styles.fullScreenModal, { backgroundColor: theme.modalBackground }]}>
            <View style={styles.modalHeader}>
              <TouchableOpacity onPress={() => setShowLanguageModal(false)} style={styles.closeModalButton}>
                <Ionicons name="close" size={30} color={theme.textColor} />
              </TouchableOpacity>
            </View>

            <View style={styles.centeredContent}>
              <Text style={[styles.modalTitle, { color: theme.textColor }]}>言語を選択してください</Text>
              <View style={styles.languageButtonsContainer}>
                {languages.map((lang) => (
                  <TouchableOpacity
                    key={lang.code}
                    style={[styles.languageSelectButton, { backgroundColor: isDarkMode ? '#444' : '#f0f0f0' }]}
                    onPress={() => handleLanguageSelect(lang.code)}
                  >
                    <Text style={styles.languageIcon}>{lang.icon}</Text>
                    <Text style={[styles.languageText, { color: theme.textColor }]}>{lang.name}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
